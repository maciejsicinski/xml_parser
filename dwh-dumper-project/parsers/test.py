    ...
    try:
        ...
        for i, expression in enumerate(expressions):
            if expression and not expression.alias and hasattr(expression, '__dict__'):
                ...
                parsed_expression = sqlglot.parse_one(new_expression)
                print(parsed_expression)
                parsed_expression = parsed_expression.find(exp.Expression)
                print(parsed_expression)
...
# Example SQL query
sql_query = "SELECT col1 aa, col2 + col3, col4 FROM table1 WHERE col5 > 10"
